 ...
 */
enum class CangjieAnnotationTarget(val description: String) {
     ...
     */
    val bitPosition: Int get() = ordinal

    /** std.core.AnnotationKind 的构造器名称，最后一项为 Extension。 */
    ...
    companion object {
         ...
         */
        val ALL_TARGETS: AnnotationTargetSet = AnnotationTargetSet.all()

        /**
         * 按官方 `ANNOTATION_TARGET_2_STRING` 序返回所有目标，供诊断序列化对齐。
         */
        fun valuesInOfficialOrder(): List<CangjieAnnotationTarget> = entries.toList()
    }
}
 ...
 */
class AnnotationTargetSet internal constructor(
     ...
     */
    val isAll: Boolean get() = enabled == null

    /**
     * 判定给定目标是否被本集合允许，对应官方 `Annotation::TestTarget(t)`。
     */
    fun matches(target: CangjieAnnotationTarget): Boolean =
        enabled?.contains(target) ?: true

    /**
     * 判定本集合是否与另一集合存在任意共同允许目标。
     */
    fun anyMatch(other: AnnotationTargetSet): Boolean {
    ...
    companion object {
        /** 创建一个仅允许给定目标的集合。 */
        fun of(targets: Iterable<CangjieAnnotationTarget>): AnnotationTargetSet =
            AnnotationTargetSet(targets.toList().toSet())

        /** 创建一个仅允许给定目标的集合。 */
        fun of(target: CangjieAnnotationTarget, vararg others: CangjieAnnotationTarget): AnnotationTargetSet =
            AnnotationTargetSet(setOf(target, *others))

        /** 创建一个启用全部目标的集合，对应官方 `EnableAllTargets()`。 */
        fun all(): AnnotationTargetSet = AnnotationTargetSet(null)

        /** 创建一个空集合，对应官方 `target = 0`（未启用任何目标）。 */
        fun empty(): AnnotationTargetSet = AnnotationTargetSet(emptySet())
    }
}
