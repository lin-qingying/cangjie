...
class LightTreeRawCfirExpressionBuilder(
    ...
    fun convertExpression(node: LighterASTNode): CfirExpression = when (node.tokenType) {
        ...
        // this / super
        CjNodeTypes.THIS_EXPRESSION -> buildQualifiedAccess {
            source = node.toSource()
            calleeReference = buildNamedReference(Name.special("<this>"))
        }
        CjNodeTypes.SUPER_EXPRESSION -> buildQualifiedAccess {
            source = node.toSource()
    ...
    private fun convertLambda(node: LighterASTNode): CfirLambdaExpression {
        ...
        val anonymousName = Name.special("<anonymous>")
        val anonymousFunction = buildSourceDeclaration(CfirFunctionSymbol(callableIdFor(anonymousName))) { symbol ->
            buildFunction {
                source = node.toSource()
                this.symbol = symbol
                origin = CfirDeclarationOrigin.Source
