...
interface LambdaAnalyzer {
    fun analyzeAndGetLambdaReturnArguments(
        ...
        expectedReturnType: ConeCangJieType?, // null means, that return type is not proper i.e. it depends on some type variables
        candidate: Candidate,
...
class PostponedArgumentsAnalyzer(
    ...
    private fun runContextSensitiveResolutionAndApplyResultsIfSuccessful(
        atom: ConeSimpleNameForContextSensitiveResolution,
        topLevelCandidate: Candidate,
        substitutedExpectedType: ConeCangJieType,
    ): Boolean {
    ...
    fun analyzeLambda(
        ...
        fun substitute(type: ConeCangJieType) = currentSubstitutor.safeSubstitute(c, type).asCone()
    ...
    fun applyResultsOfAnalyzedLambdaToCandidateSystem(
        ...
        substituteAlreadyFixedVariables: (ConeCangJieType) -> ConeCangJieType = c.createSubstituteFunctorForLambdaAnalysis(),
    ) {
    ...
    private fun PostponedArgumentsAnalyzerContext.createSubstituteFunctorForLambdaAnalysis(): (ConeCangJieType) -> ConeCangJieType {
