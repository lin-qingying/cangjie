...
) : CfirResolutionDiagnostic(CandidateApplicability.HIDDEN)

/** 实参数量与形参数量不匹配。 */
class WrongArgumentCount(
    val expectedCount: Int,
    val actualCount: Int,
) : CfirResolutionDiagnostic(CandidateApplicability.INAPPLICABLE_ARGUMENTS_MAPPING_ERROR)


/** 可见性违规，例如访问到了 `private` / `protected` 成员。 */
class VisibilityError(
    val symbol: CfirSymbol<*>,
) : CfirResolutionDiagnostic(CandidateApplicability.RESOLVED_WITH_ERROR)

/** 泛型推断中的约束系统错误。 */
class InferenceConstraintError(
    val message: String,
) : CfirResolutionDiagnostic(CandidateApplicability.INAPPLICABLE)
