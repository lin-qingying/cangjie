...
- member visibility: current official `cjc` accepts cross-package `protected` member access and accepts `internal` member access outside the defining package in this matrix, so `visibility/protectedAndInternalMatrix.cj` keeps those cases positive.
