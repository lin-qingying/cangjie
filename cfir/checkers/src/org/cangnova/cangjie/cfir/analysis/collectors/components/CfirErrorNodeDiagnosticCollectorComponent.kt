...
class CfirErrorNodeDiagnosticCollectorComponent(
     ...
     *
     * 函数调用可能因为：
     *   - 找不到被调函数          → 调用节点的类型为 [ConeErrorType]
     *   - 调用目标引用无法解析     → calleeReference 为 [CfirErrorReference]
     * 两种错误均在此处处理。
     */
    ...
    /**
     * 访问属性访问节点（形如 `obj.prop` 或单独的 `name`）。
     * 处理逻辑与函数调用相同，错误可能来自类型或引用。
     */
    ...
    /**
     * 访问限定访问节点（形如 `pkg.ClassName` 或 `Enum.Constructor`）。
     */
    ...
    /**
     * 访问错误引用节点（[CfirErrorReference]）。
     *
     * 本 visit 入口由 [processErrorReference] 负责调用，直接在此处返回即可，
     * 真正的报告逻辑由 [processErrorReference] → [processConeTypeDiagnostic] 完成。
     */
    override fun visitErrorReference(errorReference: CfirErrorReference, data: CheckerContext) {
        // 错误引用的诊断通过调用节点的 ConeErrorType 间接报告，此处不重复处理。
        return
    }
