...
class CfirCatchPatternImpl @CfirImplementationDetail constructor(
    ...
    override fun <D> transformChildren(transformer: CfirTransformer<D>, data: D): CfirCatchPatternImpl {
        ...
        transformResolvedTypeRef(transformer, data)
    ...
    override fun <D> transformResolvedTypeRef(transformer: CfirTransformer<D>, data: D): CfirCatchPatternImpl {
        resolvedTypeRef = resolvedTypeRef?.transform(transformer, data)
        return this
    }
    ...
    override fun replaceTypeRefs(newTypeRefs: List<CfirTypeRef>) {
        if (typeRefs === newTypeRefs) return
        typeRefs.clear()
        typeRefs.addAll(newTypeRefs)
    }

    override fun replaceResolvedTypeRef(newResolvedTypeRef: CfirResolvedTypeRef?) {
        resolvedTypeRef = newResolvedTypeRef
    }
}
