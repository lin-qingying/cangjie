...
private fun CjValueArgument.asPublicNamedAnnotationValue(
    session: CaCfirSession,
    token: CaLifetimeToken,
    fallbackName: Name?,
    position: Int,
 ...
 */
private fun CjExpression.asPublicAnnotationValue(
    session: CaCfirSession,
    token: CaLifetimeToken,
): CaAnnotationValue {
    when (this) {
...
private fun CjExpression.sessionExpressionClassId(session: CaCfirSession): ClassId? =
    session.queryExpressionType(this)?.classIdOrPrimitiveClassId
