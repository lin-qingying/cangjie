...
/**
 * platform-interface API surface 中的一条公开声明。
 */
data class PlatformApiSurfaceDeclaration(
    val ownerPath: String?,
    val signature: String,
)

/**
 * platform-interface API surface 中可继续承载子声明的公开容器。
 */
data class PlatformApiSurfaceContainer(
    val ownerPath: String,
)

/**
 * platform-interface 的源码级 API surface 提取器。
 *
 * 这里与 analysis-api 使用同一套声明提取规则，保证平台接口层和 Analysis API 主接口层
 * 的基线约束一致，而不是各自演化出不同的 dump 语义。
 */
class PlatformApiSurfaceExtractor(
    ...
    private fun extractDeclarations(source: String): List<PlatformApiSurfaceDeclaration> {
        val declarations = mutableListOf<PlatformApiSurfaceDeclaration>()
    ...
    private fun collectDeclarations(
        ...
        ownerPath: String?,
        bracePairs: IntArray,
        declarations: MutableList<PlatformApiSurfaceDeclaration>,
    ) {
    ...
    private data class ExtractedDeclaration(
        val declaration: PlatformApiSurfaceDeclaration,
        val container: PlatformApiSurfaceContainer?,
    )
...
/**
 * 生成 platform-interface API surface dump 的任务。
 */
abstract class GeneratePlatformApiSurfaceDumpTask : DefaultTask() {
    @get:Input
    ...
    @TaskAction
    fun generate() {
        val dumpText = PlatformApiSurfaceExtractor(sourceRoot.get().asFile).buildDump(moduleLabel.get())
...
/**
 * 校验 platform-interface API surface dump 的任务。
 */
abstract class CheckPlatformApiSurfaceDumpTask : DefaultTask() {
    @get:Input
    abstract val moduleLabel: Property<String>
    ...
    fun checkDump() {
        ...
        val actual = PlatformApiSurfaceExtractor(sourceRoot.get().asFile).buildDump(moduleLabel.get())
...
val apiSurfaceDumpFile = layout.projectDirectory.file("api/analysis-api-platform-interface.txt")
val apiSourceRoot = layout.projectDirectory.dir("src")

val generateApiSurfaceDump by tasks.registering(GeneratePlatformApiSurfaceDumpTask::class) {
    group = "verification"
    description = "生成 analysis-api-platform-interface 的公开 API surface dump。"
    moduleLabel.set("analysis-api-platform-interface")
    sourceRoot.set(apiSourceRoot)
    outputFile.set(apiSurfaceDumpFile)
}

val checkApiSurfaceDump by tasks.registering(CheckPlatformApiSurfaceDumpTask::class) {
    group = "verification"
    description = "校验 analysis-api-platform-interface 当前源码与公开 API surface dump 一致。"
    moduleLabel.set("analysis-api-platform-interface")
