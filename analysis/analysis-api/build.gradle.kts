...
/**
 * Analysis API surface dump 中的一条公开声明。
 */
data class ApiSurfaceDeclaration(
    val ownerPath: String?,
    val signature: String,
)

/**
 * Analysis API surface dump 中可继续承载子声明的公开容器。
 */
data class ApiSurfaceContainer(
    val ownerPath: String,
)

/**
 * 源码级公开 API surface 提取器。
 *
 * 该提取器服务于 analysis 框架自身的 API 基线校验，目标是稳定描述源码里真正的公开声明：
 * 1. 只提取 top-level 声明与公开容器的直接公开成员；
 * 2. 函数体、属性实现体、lazy/buildMap 等局部作用域整体跳过；
 * 3. companion object、跨行签名、带默认参数的函数头都按容器语义稳定建模。
 */
class ApiSurfaceExtractor(
    ...
    fun buildDump(moduleLabel: String): String {
        ...
        val renderedFiles = ktFiles.mapNotNull(::renderFile)
        return buildString {
    ...
    private fun extractDeclarations(source: String): List<ApiSurfaceDeclaration> {
        val declarations = mutableListOf<ApiSurfaceDeclaration>()
    ...
    private fun collectDeclarations(
        ...
        ownerPath: String?,
        bracePairs: IntArray,
        declarations: MutableList<ApiSurfaceDeclaration>,
    ) {
    ...
    private fun extractDeclaration(
        ...
        val declaration = ApiSurfaceDeclaration(
            ownerPath = ownerPath,
    ...
    private data class ExtractedDeclaration(
        val declaration: ApiSurfaceDeclaration,
        val container: ApiSurfaceContainer?,
    )
...
/**
 * 生成公开 API surface dump 的任务。
 */
abstract class GenerateApiSurfaceDumpTask : DefaultTask() {
    @get:Input
    ...
    @TaskAction
    fun generate() {
        val dumpText = ApiSurfaceExtractor(sourceRoot.get().asFile).buildDump(moduleLabel.get())
...
/**
 * 校验公开 API surface dump 与当前源码一致的任务。
 */
abstract class CheckApiSurfaceDumpTask : DefaultTask() {
    @get:Input
    abstract val moduleLabel: Property<String>
    ...
    fun checkDump() {
        ...
        val actual = ApiSurfaceExtractor(sourceRoot.get().asFile).buildDump(moduleLabel.get())
...
val apiSurfaceDumpFile = layout.projectDirectory.file("api/analysis-api.txt")
val apiSourceRoot = layout.projectDirectory.dir("src")

val generateApiSurfaceDump by tasks.registering(GenerateApiSurfaceDumpTask::class) {
    group = "verification"
    description = "生成 analysis-api 的公开 API surface dump。"
    moduleLabel.set("analysis-api")
    sourceRoot.set(apiSourceRoot)
    outputFile.set(apiSurfaceDumpFile)
}

val checkApiSurfaceDump by tasks.registering(CheckApiSurfaceDumpTask::class) {
    group = "verification"
    description = "校验 analysis-api 当前源码与公开 API surface dump 一致。"
    moduleLabel.set("analysis-api")
