...
/**
 * `analysis-api-impl-base` API surface 中的一条公开声明。
 */
data class ImplBaseApiSurfaceDeclaration(
    val ownerPath: String?,
    val signature: String,
)

/**
 * `analysis-api-impl-base` API surface 中可以继续承载子声明的公开容器。
 */
data class ImplBaseApiSurfaceContainer(
    val ownerPath: String,
)

/**
 * `analysis-api-impl-base` 的源码级 API surface 提取器。
 *
 * 这里校验的是框架稳定协议，而不是把实现细节全部倾倒出来：
 * 1. 只提取 top-level 公开声明与公开容器中的直接公开成员。
 * 2. 函数体、属性实现体、lambda、局部变量全部跳过。
 * 3. companion object、跨行签名、默认参数按容器语义稳定建模。
 */
class ImplBaseApiSurfaceExtractor(
    ...
    private fun extractDeclarations(source: String): List<ImplBaseApiSurfaceDeclaration> {
        val declarations = mutableListOf<ImplBaseApiSurfaceDeclaration>()
    ...
    private fun collectDeclarations(
        ...
        ownerPath: String?,
        bracePairs: IntArray,
        declarations: MutableList<ImplBaseApiSurfaceDeclaration>,
    ) {
    ...
    private data class ExtractedDeclaration(
        val declaration: ImplBaseApiSurfaceDeclaration,
        val container: ImplBaseApiSurfaceContainer?,
    )
...
/**
 * 生成 `analysis-api-impl-base` API surface dump 的任务。
 */
abstract class GenerateImplBaseApiSurfaceDumpTask : DefaultTask() {
    @get:Input
    ...
    @TaskAction
    fun generate() {
        val dumpText = ImplBaseApiSurfaceExtractor(sourceRoot.get().asFile).buildDump(moduleLabel.get())
...
/**
 * 校验 `analysis-api-impl-base` API surface dump 的任务。
 */
abstract class CheckImplBaseApiSurfaceDumpTask : DefaultTask() {
    @get:Input
    abstract val moduleLabel: Property<String>
    ...
    fun checkDump() {
        ...
        val actual = ImplBaseApiSurfaceExtractor(sourceRoot.get().asFile).buildDump(moduleLabel.get())
...
val apiSurfaceDumpFile = layout.projectDirectory.file("api/analysis-api-impl-base.txt")
val apiSourceRoot = layout.projectDirectory.dir("src")

val generateApiSurfaceDump by tasks.registering(GenerateImplBaseApiSurfaceDumpTask::class) {
    group = "verification"
    description = "生成 analysis-api-impl-base 的公开 API surface dump。"
    moduleLabel.set("analysis-api-impl-base")
    sourceRoot.set(apiSourceRoot)
    outputFile.set(apiSurfaceDumpFile)
}

val checkApiSurfaceDump by tasks.registering(CheckImplBaseApiSurfaceDumpTask::class) {
    group = "verification"
    description = "校验 analysis-api-impl-base 当前源码与公开 API surface dump 一致。"
    moduleLabel.set("analysis-api-impl-base")
