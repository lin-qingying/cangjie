...
internal class FileStructure private constructor(
    ...
    private fun getStructureCjElement(element: CjElement, nonLocalContainer: CjElement?): CjElement? {
        ...
        val resultedContainer = when {
            container is CjClassOrObject && container.isPartOfSuperClassCall(element) -> {
    ...
    private fun CjClassOrObject.isPartOfSuperClassCall(element: CjElement): Boolean {
    ...
    fun getAllStructureElements(): Collection<FileStructureElement> {
        ...
        ktFile.accept(object : CjVisitorVoid() {
            ...
            override fun visitDeclaration(dcl: CjDeclaration) {
                ...
                val canHaveInnerStructure = dcl is CjClassOrObject
    ...
    private fun createDeclarationStructure(declaration: CjDeclaration): FileStructureElement {
        val firDeclaration = declaration.findSourceNonLocalCfirDeclaration(firFile, firProvider)
    ...
    private fun createDanglingModifierListStructure(container: CjModifierList): FileStructureElement {
        val firDanglingModifierList = container.findSourceByTraversingWholeTree(
            moduleComponents.firFileBuilder,
            firFile,
        ) as? CfirDanglingModifierList ?: errorWithCfirSpecificEntries("No dangling modifier found", psi = container)

        firDanglingModifierList.lazyResolveToPhase(CfirResolvePhase.BODY_RESOLVE)
    ...
    private fun createStructureElement(container: CjElement): FileStructureElement = when (container) {
        ...
        is CjModifierList -> createDanglingModifierListStructure(container)
