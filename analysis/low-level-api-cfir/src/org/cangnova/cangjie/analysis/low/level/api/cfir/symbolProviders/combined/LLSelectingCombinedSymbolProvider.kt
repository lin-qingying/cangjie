...
/**
 * A combined symbol provider which *selects* a subset of its [providers] to delegate to by an element's [CaModule]. For example, a
 ...
 *
 * Classpath order must be preserved with [selectCfirstElementInClasspathOrder] in case a single result is required.
 */
internal abstract class LLSelectingCombinedSymbolProvider<PROVIDER : CfirSymbolProvider>(
    ...
    /**
     * Maps each [CaModule] (corresponding uniquely to a symbol provider) to its index in [providers]. This determines the [CaModule]
     ...
     */
    protected inline fun <CANDIDATE> selectCfirstElementInClasspathOrder(
