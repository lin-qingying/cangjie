...
/**
 * `.cjo` loaded package -> compiled stub / decompiled text 的物化器。
 */
object CaDecompiledPackageViewBuilder {
    /**
     * 直接按已加载的 `.cjo` 包和给定 moduleData 物化 package view。
     */
    fun buildPackageView(
        ...
        if (!loadedPackage.isVersionSupported) {
            return CaDecompiledPackageView(
                ...
                declarationDebug = emptyList(),
            )
        }

        return runCatching {
            val declarations = CaCjoDeclarationLoader.loadDeclarations(loadedPackage, moduleData)
            CaDecompiledPackageView(
                owningBinaryFile = loadedPackage.binaryFile,
                packageFqName = loadedPackage.packageFqName,
                ...
                declarationDebug = declarations.map { declaration ->
                    val debugName = when (declaration) {
                        ...
                        is CfirNamedFunction -> declaration.name.asString()
                        is CfirMacroDeclaration -> declaration.name.asString()
                        is CfirProperty -> declaration.name.asString()
                        is CfirFieldVariable -> declaration.name.asString()
                        is CfirPatternVariable -> declaration.symbol.callableId.callableName.asString()
                        is CfirMainFunction -> MAIN.asString()
                        is CfirExtend -> renderTypeRef(declaration.extendedTypeRef)
                        else -> declaration::class.simpleName.orEmpty()
                    }
                    "${declaration::class.qualifiedName}:$debugName"
                },
            )
        }.getOrElse { throwable ->
    ...
    private fun createExtendStub(
        ...
        declaration.declarations.forEach { child ->
            createDeclarationStub(child, bodyStub, packageFqName, null, extendName)
        }
    }
    ...
    private fun createClassStub(
        ...
        val bodyStub = createTypeStatementBodyStub(stub, CjStubElementTypes.CLASS_BODY)
        declaration.declarations.forEach { child ->
            createDeclarationStub(
                child,
                bodyStub,
                packageFqName,
                composeQualifiedName(packageFqName, owningClassFqName, declaration.name),
    ...
    private fun createInterfaceStub(
        ...
        val bodyStub = createTypeStatementBodyStub(stub, CjStubElementTypes.INTERFACE_BODY)
        declaration.declarations.forEach { child ->
            createDeclarationStub(child, bodyStub, packageFqName, qualifiedName, declaration.name.asString())
        }
    }
    ...
    private fun createEnumStub(
        ...
        val bodyStub = createTypeStatementBodyStub(stub, CjStubElementTypes.ENUM_BODY)
        declaration.declarations.forEach { child ->
            createDeclarationStub(child, bodyStub, packageFqName, qualifiedName, declaration.name.asString())
        }
    }
...
data class CaDecompiledPackageView(
    ...
    val declarationDebug: List<String>,
)
