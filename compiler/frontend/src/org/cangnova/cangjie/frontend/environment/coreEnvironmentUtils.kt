...
fun List<CangJieSourceRoot>.forAllFiles(
    ...
    allSourceFilesSequence(
        ...
        filter = { virtualFile, isExplicit ->
            // 非仓颉输入先允许插件更新文件注册，再重新分类；模式选择与分组收集器同源。
            if (virtualFile.cangjieSourceKind() == null) {
                ensurePluginsConfigured()
            }
            val isCangJie = virtualFile.cangjieSourceKind()?.let(configuration::acceptsCangjieSource) == true
