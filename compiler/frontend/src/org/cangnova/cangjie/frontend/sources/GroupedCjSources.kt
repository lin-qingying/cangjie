        ...
        .allSourceFilesSequence(
            ...
            filter = { virtualFile, isExplicit ->
                ...
                when {
                    ...
                    // 与 forAllFiles 共用种类识别及输入互斥策略。
                    sourceKind != null -> compilerConfiguration.acceptsCangjieSource(sourceKind)

                    // 非仓颉文件仍走既有插件配置与显式输入诊断。
                    else -> {
                        if (virtualFile.isFile) {
                            ensurePluginsConfigured()
                            val isCangJie = virtualFile.cangjieSourceKind()
                                ?.let(compilerConfiguration::acceptsCangjieSource) == true
            ...
            convertToSourceFiles = { virtualFile ->
                ...
                if (virtualFile.extension == CangJieFileType.EXTENSION || virtualFile.cangjieSourceKind() == CjSourceKind.DECLARATION) {
                    sources
                } else {
