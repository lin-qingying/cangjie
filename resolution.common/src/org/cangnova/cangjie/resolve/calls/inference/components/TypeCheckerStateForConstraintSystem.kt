...
abstract class TypeCheckerStateForConstraintSystem(
        ...
        val lowerConstraint = when (typeVariable) {
            is RigidTypeMarker ->
                when {
                     ...
                     * ```
                     * In 2.1 (see [org.cangnova.cangjie.config.LanguageFeature.InferenceEnhancementsIn21])
    ...
    private fun addForkPointForDifferentDnnAndMarkedNotNullable(
        subType: CangJieTypeMarker,
        notNullSubType: CangJieTypeMarker,
    ...
    private fun CangJieTypeMarker.withCapturedNonNullProjection(): CangJieTypeMarker =
...
//      TODO: rethink, maybe we should take nullability into account somewhere else
    ...
    private inline fun CangJieTypeMarker.bothBounds(f: (RigidTypeMarker) -> Boolean) = when (this) {
        is RigidTypeMarker -> f(this)
        is FlexibleTypeMarker -> with(typeSystemContext) { f(lowerBound()) && f(upperBound()) }
        else -> error("sealed")
    }

    private inline fun CangJieTypeMarker.anyBound(f: (RigidTypeMarker) -> Boolean) = when (this) {
        is RigidTypeMarker -> f(this)
        is FlexibleTypeMarker -> with(typeSystemContext) { f(lowerBound()) || f(upperBound()) }
        else -> error("sealed")
    }
